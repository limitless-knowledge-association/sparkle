...
describe('Git Operations Integration', () => {
  ...
  describe('Cross-clone synchronization', () => {
    test('item visibility via pull', async () => {
      ...
      const itemId = await sparkle1.createItem('Item from clone1');
      ...
      console.log(`  ✓ Item visible in clone2 after pull`);
    }, 90000);

    test('concurrent entries merge correctly', async () => {
      const env = await setupTestEnv('test4-merge', 2);
      const clone1 = env.cloneSetups[0];
      const clone2 = env.cloneSetups[1];

      // Create item in clone1
      const sparkle1 = await createSparkleInstance(clone1);
      const itemId = await sparkle1.createItem('Shared item');
      await sleep(6000);
      await sparkle1.gitOps.commitAndPush();
      ...
      console.log(`  ✏️  Both clones added entries`);
      ...
      const item = await sparkle1.getItemDetails(itemId);
      expect(item.entries.length).toBe(2);
      console.log(`  ✓ Both entries merged correctly`);
    }, 90000);
  });

  describe('Aggregate management', () => {
    test('aggregate invalidation after pull', async () => {
      const env = await setupTestEnv('test5-aggregate', 2);
      const clone1 = env.cloneSetups[0];
      const clone2 = env.cloneSetups[1];

      const sparkle1 = await createSparkleInstance(clone1);

      // Create and modify item in clone1
      const itemId = await sparkle1.createItem('Test item', 'incomplete');
      await sparkle1.alterTagline(itemId, 'Updated tagline');
      await sleep(6000);
      await sparkle1.gitOps.commitAndPush();
      ...
      console.log(`  ✓ Aggregate rebuilt with correct data`);
    }, 90000);
  });
